...
using Kooboo.Common.Web.WebResourceLoader.Configuration;
using Kooboo.Common.Web.WebResourceLoader.DynamicClientResource;


namespace Kooboo.Common.Web.WebResourceLoader
{
            ...
            foreach (KeyValuePair<Condition, IList<FileInfoElement>> keyValuePair in conditions)
            {
                ...
                switch (section.Mode)
                {
                    case Mode.Release:
                        ...
                        {
                            url = Kooboo.Common.Web.UrlUtility.ToHttpAbsolute(baseUrl, url);
                        }
                    ...
                    case Mode.Debug:
                        foreach (FileInfoElement fileInfo in files)
                        {
                            ...
                            if (!string.IsNullOrEmpty(baseUrl))
                            {
                                fileUrl = Kooboo.Common.Web.UrlUtility.ToHttpAbsolute(baseUrl, fileUrl);
                            }
